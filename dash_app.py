...
def get_memory_usage() -> Dict[str, float]:
    """
    Get current memory usage information (Phase 3-2).
...
def log_performance(operation: str, duration: float, metadata: Dict[str, Any] = None) -> None:
    """
    Log performance metrics for an operation (Phase 3-2).
    ...
    if duration > PERFORMANCE_THRESHOLD_CRITICAL:
        level = "CRITICAL"
        log.warning(f"[性能] {level}: {operation} took {duration:.2f}s (metadata: {metadata})")
    elif duration > PERFORMANCE_THRESHOLD_WARNING:
        level = "WARNING"
        log.info(f"[性能] {level}: {operation} took {duration:.2f}s (metadata: {metadata})")
    else:
        log.debug(f"[性能] OK: {operation} took {duration:.2f}s (metadata: {metadata})")
...
def _build_scenario_data(path: Path) -> ScenarioData:
    """Build ScenarioData from a scenario directory path."""
...
# ============================================================================
# END SESSION MANAGEMENT INFRASTRUCTURE
# ============================================================================
...
# Thread-local storage for scenario directory path and session ID (fixes race condition in multi-threaded environment)
...
def _get_current_session_id() -> str | None:
    """Get the current thread's session ID for cache isolation."""
    return getattr(_thread_local, 'CURRENT_SESSION_ID', None)

def _set_current_session_id(session_id: str | None) -> None:
...
def data_get(key: str, default=None, for_display: bool = False):
    ...
    Uses session-and-scenario-isolated cache keys to prevent data mixing between different sessions and scenarios.
    Cache key format: "<session_id>_<scenario_name>_<key>" or "<session_id>_<key>" or "<key>"
    """
    # Get current session ID and scenario directory for cache isolation
    session_id = _get_current_session_id()
    current_dir = _get_current_scenario_dir()

    # Extract scenario name from directory path (e.g., "out_mean_based")
    scenario_name = current_dir.name if current_dir else None

    # Create session-and-scenario-isolated cache key
    if session_id and scenario_name:
        cache_key = f"{session_id}_{scenario_name}_{key}"
    elif session_id:
        cache_key = f"{session_id}_{key}"
    else:
        cache_key = key

    log.debug(f"data_get('{key}'): キャッシュを検索中... (cache_key={cache_key}, scenario={scenario_name})")

    # キャッシュチェック（ThreadSafeLRUCacheを使用）
    cached_value = DATA_CACHE.get(cache_key)
    if cached_value is not None:
        log.debug(f"data_get('{key}'): キャッシュヒット (session_id={session_id}, scenario={scenario_name})")
        return cached_value
    ...
    if key == "need_per_date_slot":
        ...
        DATA_CACHE.set(cache_key, empty_df)
        return empty_df
    ...
    if key == "summary_report":
        files = sorted(current_dir.glob("OverShortage_SummaryReport_*.md"))
        if files:
            text = files[-1].read_text(encoding="utf-8")
            DATA_CACHE.set(cache_key, text)
            log.debug(f"Loaded summary report {files[-1]}")
            return text
    if key in {"roles", "employments"}:
        roles, employments = load_shortage_meta(current_dir)
        # Create session-and-scenario-isolated cache keys for roles and employments
        if session_id and scenario_name:
            roles_cache_key = f"{session_id}_{scenario_name}_roles"
            employments_cache_key = f"{session_id}_{scenario_name}_employments"
        elif session_id:
            roles_cache_key = f"{session_id}_roles"
            employments_cache_key = f"{session_id}_employments"
        else:
            roles_cache_key = "roles"
            employments_cache_key = "employments"
        DATA_CACHE.set(roles_cache_key, roles)
        DATA_CACHE.set(employments_cache_key, employments)
        return DATA_CACHE.get(cache_key, default)

    if key == "shortage_events":
        df_events = over_shortage_log.list_events(current_dir)
        DATA_CACHE.set(cache_key, df_events)
        # Create session-and-scenario-isolated cache key for shortage_log_path
        if session_id and scenario_name:
            shortage_log_path_key = f"{session_id}_{scenario_name}_shortage_log_path"
        elif session_id:
            shortage_log_path_key = f"{session_id}_shortage_log_path"
        else:
            shortage_log_path_key = "shortage_log_path"
        DATA_CACHE.set(shortage_log_path_key, str(Path(current_dir) / "over_shortage_log.csv"))
        return DATA_CACHE.get(cache_key, default)

    # 🎯 高度分析結果の読み込み (app.py統合機能)
    if key == "advanced_analysis":
        advanced_results = load_advanced_analysis_results(current_dir)
        DATA_CACHE.set(cache_key, advanced_results)
        return advanced_results

    if key == "forecast_data":
        forecast_file = current_dir / "forecast.parquet"
        if forecast_file.exists():
            forecast_df = safe_read_parquet(forecast_file)
            DATA_CACHE.set(cache_key, forecast_df)
            return forecast_df
    
    if key == "mind_reader_analysis":
        # Mind Reader分析結果をキャッシュから取得または実行
        # Session-isolated cache key for mind_reader
        mind_reader_cache_key = f"{session_id}_mind_reader_{get_data_hash()}" if session_id else f"mind_reader_{get_data_hash()}"
        cached_result = DATA_CACHE.get(mind_reader_cache_key)
        if cached_result is not None:
            return cached_result

        # リアルタイム分析実行（タイムアウト付き - Windows互換実装）
        ...
        if long_df is not None and not long_df.empty:
            ...
            try:
                # Windows互換のタイムアウト設定（30秒）- concurrent.futuresを使用
                from concurrent.futures import ThreadPoolExecutor, TimeoutError as FuturesTimeoutError

                def run_mind_reader():
                    return mind_reader.read_creator_mind(long_df)

                with ThreadPoolExecutor(max_workers=1) as executor:
                    future = executor.submit(run_mind_reader)
                    try:
                        mind_results = future.result(timeout=30)  # 30秒でタイムアウト
                        DATA_CACHE.set(mind_reader_cache_key, mind_results)
                        return mind_results
    ...
    DATA_CACHE.set(cache_key, default)
    return default
...
def create_overview_tab(selected_scenario: str = None) -> html.Div:
    ...
    if ComprehensiveDashboard is not None and current_dir is not None:
        try:
...
def _generate_static_heatmap(selected_role: str, selected_emp: str) -> dcc.Graph:
    """
    指定された職種・雇用形態でヒートマップを静的に生成（Pattern A）

    Args:
        selected_role: 職種フィルタ値（'all'または具体的な職種名）
        selected_emp: 雇用形態フィルタ値（'all'または具体的な雇用形態名）

    Returns:
        dcc.Graph: ヒートマップグラフコンポーネント
    """
    # 1. データ取得
    ...
    if aggregated_df is None or aggregated_df.empty:
        ...
        empty_fig = go.Figure().update_layout(
            title_text="ヒートマップの元データが見つかりません",
            height=300
        )
    ...
    if 'staff_count' in filtered_df.columns:
        ...
        if before_count != after_count:
            log.info(f"[Heatmap Static] 休日除外フィルタ適用: {before_count} -> {after_count} ({before_count - after_count}件除外)")
    ...
    if filtered_df.empty:
        ...
        log.info(f"[Heatmap Static] 空データで連続した{len(all_dates)}日の日付軸を表示")
    ...
    try:
        ...
        if 'dates' in meta_data and meta_data['dates']:
            ...
            log.info(f"[Heatmap Static] メタデータから日付範囲取得: {len(all_dates_to_use)}日")
        else:
    ...
    if all_dates_to_use:
        ...
        log.info(f"[Heatmap Static] 全期間でヒートマップ作成: {len(all_dates_to_use)}日（連続した日付軸で表示）")
    else:
        log.warning(f"[Heatmap Static] '{title}': 日付範囲データが見つかりません")
...
def create_heatmap_tab() -> html.Div:
    ...
    default_emp1 = employments[0] if len(employments) > 0 else 'all'

    default_role2 = roles[1] if len(roles) > 1 else (roles[0] if len(roles) > 0 else 'all')
    default_emp2 = employments[1] if len(employments) > 1 else (employments[0] if len(employments) > 0 else 'all')

    log.info(f"[Heatmap] デフォルト値: エリア1=({default_role1}, {default_emp1}), エリア2=({default_role2}, {default_emp2})")

    # 初期表示用のヒートマップ生成
    heatmap1 = _generate_static_heatmap(default_role1, default_emp1)
    heatmap2 = _generate_static_heatmap(default_role2, default_emp2)

    # フィルターオプションの構築
    role_options = [{'label': 'すべて', 'value': 'all'}] + [{'label': r, 'value': r} for r in roles]
    emp_options = [{'label': 'すべて', 'value': 'all'}] + [{'label': e, 'value': e} for e in employments]
    ...
    return html.Div([
        ...
        html.Div([
            html.P([
                html.Strong("✨ Phase 2実装: ", style={'color': '#1976d2'}),
                "職種・雇用形態フィルターで動的に比較分析できます。",
                html.Br(),
                "2つのエリアで異なる条件のヒートマップを並べて比較できます。"
            ], style={
                'padding': '10px',
        ...
        html.Details([
            ...
            html.Div([
                ...
                **比較分析:**
                - 上下2エリアで異なる条件のヒートマップを比較
                - 同一時間軸での需給パターン比較
                - インタラクティブフィルター: 職種・雇用形態別の詳細分析
                """),
        ...
        html.Div([
            ...
            # ヒートマップ表示エリア (Phase 2+ Loading付き)
            dcc.Loading(
                id="loading-heatmap-area1",
                type="circle",
                children=html.Div(id='heatmap-area1', children=heatmap1)
            )
        ], style={'padding': '10px', 'backgroundColor': '#f9f9f9', 'borderRadius': '5px', 'marginBottom': '10px'}),

        # 比較エリア2
        html.Div([
            html.H4("比較エリア 2", style={
                'marginTop': '20px',
                'borderTop': '2px solid #ddd',
                'paddingTop': '20px',
                'color': '#1976d2'
            ...
            # ヒートマップ表示エリア (Phase 2+ Loading付き)
            dcc.Loading(
                id="loading-heatmap-area2",
                type="circle",
                children=html.Div(id='heatmap-area2', children=heatmap2)
            )
        ], style={'padding': '10px', 'backgroundColor': '#f9f9f9', 'borderRadius': '5px', 'marginBottom': '10px'})
    ])
...
def create_shortage_tab(selected_scenario: str = None) -> html.Div:
    ...
    try:
        ...
        # 職種×雇用形態クロス集計表（ヒートマップタブと同様のパターン）
        content.append(html.H4("職種×雇用形態別 勤務実績", style={'marginTop': '30px'}))  # type: ignore
        content.append(html.P("各職種と雇用形態の組み合わせごとの総勤務時間を表示します。"))  # type: ignore

        try:
            pre_agg = data_get('pre_aggregated_data', pd.DataFrame())
            roles_list = data_get('roles', [])
            employments_list = data_get('employments', [])

            if not pre_agg.empty and 'role' in pre_agg.columns and 'employment' in pre_agg.columns and 'staff_count' in pre_agg.columns:
                # 職種×雇用形態でグループ化して総勤務人数（スロット数）を集計
                cross_tab_data = pre_agg.groupby(['role', 'employment'])['staff_count'].sum().reset_index()

                # スロット数を時間に変換
                cross_tab_data['total_hours'] = cross_tab_data['staff_count'] * SLOT_HOURS

                # ピボットテーブル形式に変換
                pivot_table = cross_tab_data.pivot(index='role', columns='employment', values='total_hours').fillna(0)
                ...
                table_df.columns.name = None  # カラム名のインデックス名を削除

                # 数値列をフォーマット
                for col in table_df.columns[1:]:  # 最初の列（role）以外
                    table_df[col] = table_df[col].apply(lambda x: f"{x:.1f}h" if x > 0 else "-")

                content.append(dash_table.DataTable(
                    data=table_df.to_dict('records'),
                    columns=[{'name': str(col), 'id': str(col)} for col in table_df.columns],
                ...
                content.append(html.P(
                    "※ この表は実際の勤務実績を示しています。不足分析は上記の職種別・雇用形態別グラフをご参照ください。",
                    style={'marginTop': '10px', 'fontSize': '0.9rem', 'color': '#666'}
                ))  # type: ignore

                log.info(f"職種×雇用形態クロス集計表作成完了: {pivot_table.shape}")
            else:
                if pre_agg.empty:
                    content.append(html.P("詳細データ (pre_aggregated_data) が見つかりません。"))  # type: ignore
                else:
                    content.append(html.P(f"必要なカラムが不足しています。利用可能なカラム: {list(pre_agg.columns)}"))  # type: ignore
        except Exception as e:
            log.error(f"職種×雇用形態クロス集計表作成エラー: {e}")
            content.append(html.P(f"クロス集計表作成エラー: {str(e)}", style={'color': 'red'}))  # type: ignore
...
def _generate_optimization_content(scope='overall', detail_values=None) -> html.Div:
    """
    最適化分析コンテンツを生成（Pattern A対応）

    Args:
        scope: 表示範囲（'overall', 'role', 'employment'）
        detail_values: 詳細フィルタ値（scope='role'/'employment'時に使用）

    Returns:
        html.Div: 完全な最適化分析コンテンツ
    """
    # 1. heat値取得（scope別のキー組み立て）
    key_suffix = ''
    ...
    try:
        if scope == 'overall':
            ...
            if not need_per_date_slot_df.empty:
                ...
                log.info(f"[Optimization] Using need_per_date_slot: {need_per_date_slot_df.shape}")
                ...
                if common_dates:
                    ...
                    if missing_dates:
                        log.warning(f"[Optimization] Missing dates, using fallback: {missing_dates}")
                        fallback_need = pd.DataFrame(
                            np.repeat(df_heat['need'].values[:, np.newaxis], len(missing_dates), axis=1),
                            index=df_heat.index,
                    ...
                    log.warning("[Optimization] No matching dates, falling back to average need")
                    need_df = pd.DataFrame(
                ...
                log.info("[Optimization] Using average need values")
                need_df = pd.DataFrame(
                    np.repeat(df_heat['need'].values[:, np.newaxis], len(date_cols), axis=1),
                    index=df_heat.index,
                    columns=date_cols
        ...
        log.error(f"[Optimization] need値計算エラー: {e}")
        return html.Div(f"need値計算エラー: {str(e)}")
    ...
    try:
        ...
        log.error(f"[Optimization] グラフ生成エラー: {e}")
        return html.Div(f"グラフ生成エラー: {str(e)}")
...
def create_optimization_tab() -> html.Div:
    """
    最適化分析タブを作成（Pattern A）

    Pattern A実装:
    - デフォルト: scope='overall'で初期表示
    - ドロップダウン変更時に動的更新
    """
    # デフォルトコンテンツ生成（scope='overall'）
    log.info("[Optimization] Pattern A: Generating default content (scope=overall)")
    default_content = _generate_optimization_content(scope='overall', detail_values=None)
    ...
    return html.Div([  # type: ignore
        ...
        html.Div(
            ...
            children=default_content  # ← Pattern A: 初期コンテンツを設定
        )  # type: ignore
    ])
...
def _generate_cost_analysis_content(by_key='role', wages=None) -> html.Div:
    """
    コスト分析コンテンツを生成（Pattern A対応）

    Args:
        by_key: 分析軸（'role', 'employment', 'staff'）
        wages: 単価辞書（省略時はデフォルト単価を使用）

    Returns:
        html.Div: 完全なコスト分析コンテンツ
    """
    long_df = data_get('long_df')
    ...
    try:
        ...
        log.error(f"[Cost] コスト計算エラー: {e}")
        return html.Div(f"コスト計算エラー: {str(e)}")
...
def create_cost_analysis_tab() -> html.Div:
    """
    コスト分析タブを作成（Pattern A）

    Pattern A実装:
    - デフォルト: by_key='role'、デフォルト単価で初期表示
    - ドロップダウン変更時に動的更新
    """
    log.info("[Cost] Pattern A: Generating default content (by_key=role)")
    default_content = _generate_cost_analysis_content(by_key='role')
    ...
    return html.Div([
        ...
        dcc.Loading(
            ...
            children=html.Div(
                ...
                children=default_content  # ← Pattern A: 初期コンテンツ設定
            )
        )
    ])
...
# ========================================
# Individual Analysis Helper Functions (Pattern A)
# ========================================

def _create_work_distribution_chart(selected_staff: str, staff_df: pd.DataFrame) -> go.Figure:
    """
    勤務区分ごとの占有割合（Pie chart）を生成
    ...
    Returns:
        go.Figure: 勤務割合の円グラフ
    """
    fig = go.Figure(layout={'title': {'text': f'{selected_staff}さんの勤務割合'}})
...
def _calculate_scores(
    ...
    Returns:
        tuple: (疲労スコア, 不公平感スコア, スコア詳細DataFrame)
    """
    fatigue_score = "データなし"
    ...
    return fatigue_score, unfairness_score, score_details_df


def _calculate_coworker_ranking(
    selected_staff: str,
    ...
    Returns:
        pd.DataFrame: 共働ランキング（Top 5）
    """
    coworker_ranking_df = pd.DataFrame()

    if not staff_df.empty and not long_df.empty:
        my_slots = staff_df[['ds']].drop_duplicates()
        coworkers = long_df[
            long_df['ds'].isin(my_slots['ds']) & (long_df['staff'] != selected_staff)
        ]
...
def _calculate_contribution(
    ...
    Returns:
        tuple: (不足時間への貢献時間, 過剰時間への貢献時間)
    """
    shortage_contribution_h = 0.0
    excess_contribution_h = 0.0

    if staff_df.empty:
        return shortage_contribution_h, excess_contribution_h

    # 勤務スロットの準備
    staff_work_slots = staff_df[staff_df.get('parsed_slots_count', 0) > 0][['ds']].copy()
    if staff_work_slots.empty:
        return shortage_contribution_h, excess_contribution_h
    ...
    if not shortage_df.empty:
        try:
            shortage_long = shortage_df.melt(
                var_name='date_str',
            ...
            log.warning(f"[Individual] 不足貢献度計算エラー: {e}")

    # 人員過剰への貢献
    if not excess_df.empty:
        try:
            excess_long = excess_df.melt(
                var_name='date_str',
                value_name='excess_count',
                ignore_index=False
            ).reset_index().rename(columns={'index': 'time'})
            ...
            log.warning(f"[Individual] 過剰貢献度計算エラー: {e}")

    return shortage_contribution_h, excess_contribution_h


def _create_leave_tendency_chart(selected_staff: str, staff_df: pd.DataFrame) -> go.Figure:
    """
    休暇取得傾向のBar chartを生成
    ...
    Returns:
        go.Figure: 曜日別休暇取得のBar chart
    """
    fig = go.Figure(layout={'title': {'text': '曜日別の休暇取得日数'}})
...
def _calculate_work_patterns(selected_staff: str, staff_df: pd.DataFrame) -> tuple[float, float]:
    """
    働き方のクセ（マンネリ度・リズム破壊度）を計算
    ...
    Returns:
        tuple: (マンネリ度スコア, リズム破壊度スコア)
    """
    mannelido = 0.0
    rhythm = 0.0

    if staff_df.empty or 'code' not in staff_df.columns:
        return mannelido, rhythm
    ...
    try:
        ...
        if not work_records.empty:
            ...
            if len(code_counts) > 0:
                mannelido = (code_counts.iloc[0] / len(work_records)) * 100
            ...
            if 'ds' in work_records.columns:
                ...
                if len(unique_dates) > 1:
                    ...
                    for i in range(1, len(unique_dates)):
                        if (pd.Timestamp(unique_dates[i]) - pd.Timestamp(unique_dates[i-1])).days == 1:
                            current_run += 1
                        else:
                    ...
                    if len(consecutive_runs) > 0:
                        rhythm = float(pd.Series(consecutive_runs).std())
    except Exception as e:
        log.warning(f"[Individual] 働き方パターン計算エラー: {e}")

    return mannelido, rhythm


def _generate_individual_basic_analysis(selected_staff: str) -> html.Div:
    """
    個別職員の基本分析を生成（Pattern A: シナジー分析除く）

    Args:
        selected_staff: 分析対象職員

    Returns:
        html.Div: 基本分析のHTMLレイアウト
    """
    # データ取得
    long_df = data_get('long_df', pd.DataFrame())
    ...
    if long_df.empty:
        return html.Div("勤務データが見つかりません。", style={'color': 'red'})

    staff_df = long_df[long_df['staff'] == selected_staff].copy()

    if staff_df.empty:
        return html.Div(f"職員 '{selected_staff}' のデータが見つかりません。", style={'color': 'orange'})
    ...
    return html.Div([
        ...
        # 2. 疲労度・不公平感・働き方のクセ
        html.Div([
            html.H4("疲労度・不公平感・働き方のクセ", style={'borderBottom': '2px solid #ddd', 'paddingBottom': '10px'}),
            html.Div([
                # メトリックカード
                html.Div([
                    html.Div([
                        html.H5("疲労スコア", style={'margin': '0', 'color': '#666'}),
                        html.H3(fatigue_score, style={'margin': '10px 0', 'color': '#d32f2f'})
                    ...
                    html.Div([
                        html.H5("不公平感スコア", style={'margin': '0', 'color': '#666'}),
                        html.H3(unfairness_score, style={'margin': '10px 0', 'color': '#f57c00'})
                    ], className="metric-card", style={
                        'padding': '15px',
                        'backgroundColor': '#fff3e0',
                        'borderRadius': '5px',
                    ...
                    html.Div([
                        html.H5("マンネリ度", style={'margin': '0', 'color': '#666'}),
                        html.H3(f"{mannelido:.1f}%", style={'margin': '10px 0', 'color': '#1976d2'})
                    ], className="metric-card", style={
                        'padding': '15px',
                        'backgroundColor': '#e3f2fd',
                        'borderRadius': '5px',
                    ...
                    html.Div([
                        html.H5("リズム破壊度", style={'margin': '0', 'color': '#666'}),
                        html.H3(f"{rhythm:.2f}", style={'margin': '10px 0', 'color': '#388e3c'})
                    ], className="metric-card", style={
                        'padding': '15px',
                ...
                # スコア詳細テーブル
                html.Div([
                    html.H5("不公平感の内訳", style={'marginTop': '10px'}),
                    dash_table.DataTable(
                        data=score_details_df.to_dict('records') if not score_details_df.empty else [],
                        columns=[{"name": i, "id": i} for i in score_details_df.columns] if not score_details_df.empty else [],
                        style_table={'overflowX': 'auto'},
                        style_cell={'textAlign': 'left', 'padding': '10px'},
                        style_header={'backgroundColor': '#f5f5f5', 'fontWeight': 'bold'}
                    ) if not score_details_df.empty else html.P("詳細データなし", style={'color': '#999'})
                ])
            ])
        ], style={'marginBottom': '20px', 'padding': '15px', 'backgroundColor': '#fafafa', 'borderRadius': '5px'}),

        # 3. 共働ランキング
        html.Div([
            html.H4("よく一緒に働く職員 Top 5", style={'borderBottom': '2px solid #ddd', 'paddingBottom': '10px'}),
            dash_table.DataTable(
                data=coworker_ranking_df.to_dict('records') if not coworker_ranking_df.empty else [],
                columns=[{"name": i, "id": i} for i in coworker_ranking_df.columns] if not coworker_ranking_df.empty else [],
                style_table={'overflowX': 'auto'},
                style_cell={'textAlign': 'left', 'padding': '10px'},
                style_header={'backgroundColor': '#e3f2fd', 'fontWeight': 'bold'},
                style_data_conditional=[
                    {'if': {'row_index': 0}, 'backgroundColor': '#fff9c4'}
                ]
            ) if not coworker_ranking_df.empty else html.P("共働データなし", style={'color': '#999'})
        ], style={'marginBottom': '20px', 'padding': '15px', 'backgroundColor': '#fafafa', 'borderRadius': '5px'}),

        # 4. 人員不足/過剰への貢献度
        html.Div([
            html.H4("人員配置への貢献", style={'borderBottom': '2px solid #ddd', 'paddingBottom': '10px'}),
            html.Div([
                html.Div([
                    html.H5("人員不足時の貢献", style={'margin': '0', 'color': '#666'}),
                    html.H3(f"{shortage_h:.1f}時間", style={'margin': '10px 0', 'color': '#d32f2f'}),
                    html.P("不足していた時間帯に勤務した合計時間", style={'fontSize': '12px', 'color': '#999', 'margin': '0'})
                ], style={
                    'padding': '15px',
                    'backgroundColor': '#ffebee',
                    'borderRadius': '5px',
                ...
                html.Div([
                    html.H5("人員過剰時の貢献", style={'margin': '0', 'color': '#666'}),
                    html.H3(f"{excess_h:.1f}時間", style={'margin': '10px 0', 'color': '#1976d2'}),
                    html.P("過剰だった時間帯に勤務した合計時間", style={'fontSize': '12px', 'color': '#999', 'margin': '0'})
                ], style={
                    'padding': '15px',
                    'backgroundColor': '#e3f2fd',
                    'borderRadius': '5px',
...
def create_individual_analysis_tab() -> html.Div:
    ...
    Pattern A実装:
    - デフォルト職員の基本分析を即座に表示
    - シナジー分析はPhase 2に延期（Phase 1では情報UIのみ）
    """
    long_df = data_get('long_df', pd.DataFrame())
    ...
    # デフォルト職員の基本分析を生成
    default_content = (
        _generate_individual_basic_analysis(default_staff)
        if default_staff
        else html.Div("職員データがありません。", style={'color': 'red'})
    )
...
def _generate_team_analysis_content(selected_key: str, selected_value: str) -> html.Div:
    """
    チーム分析コンテンツを生成（Pattern A対応）

    Args:
        selected_key: フィルタ条件のキー ('role', 'code', 'employment')
        selected_value: フィルタ条件の値

    Returns:
        html.Div: 完全なチーム分析コンテンツHTML
    """
    # データ取得
    long_df = data_get('long_df', pd.DataFrame())
    fatigue_df = data_get('fatigue_score', pd.DataFrame())
    ...
    # エラーハンドリング: 空DataFrame
    if team_df.empty:
        return html.Div([
            html.H4(f"チーム「{selected_value}」の分析結果"),
            html.P(
                "この条件に合致するチームデータが見つかりませんでした。",
                style={'color': 'orange', 'fontWeight': 'bold'}
            ),
            html.P("考えられる原因:"),
            html.Ul([
                html.Li("選択した条件に該当するスタッフが勤務していない"),
                html.Li("疲労度・不公平感データが不足している"),
            ]),
            html.P("別の条件を選択してください。")
        ])
    ...
    try:
        ...
        fig_fairness = px.line(
            ...
            labels={
                'avg_unfairness': '平均不公平感',
                'std_unfairness': '不公平感のばらつき',
                'index': '日付'
            }
        )
    except Exception as e:
        log.error(f"[Team] グラフ生成エラー: {e}")
        return html.Div([
            html.H4(f"チーム「{selected_value}」の分析結果"),
            html.P(
                f"グラフ生成中にエラーが発生しました: {str(e)}",
                style={'color': 'red', 'fontWeight': 'bold'}
            ),
            html.P("データの形式を確認してください。")
        ])
...
def create_team_analysis_tab() -> html.Div:
    """
    チーム分析タブを作成（Pattern A）

    Pattern A実装:
    - デフォルトチーム条件（key='code', value=最頻値）で初期表示
    - ドロップダウン変更時にコンテンツを動的更新
    """
    long_df = data_get('long_df', pd.DataFrame())
    if long_df.empty:
    ...
    if default_key in long_df.columns and not long_df[default_key].empty:
        mode_values = long_df[default_key].mode()
        if not mode_values.empty:
            default_value = mode_values.iloc[0]
        else:
            default_value = long_df[default_key].iloc[0]
    else:
        # fallback: 最初の利用可能な列と値
        for col in filterable_cols:
            if col in long_df.columns and not long_df[col].empty:
                default_key = col
    ...
    # Phase 2+: valueドロップダウンの初期オプション生成
    ...
    if default_value is not None:
        default_content = _generate_team_analysis_content(default_key, default_value)
    else:
        default_content = html.Div(
            "チーム分析に必要なデータが不足しています。",
            style={'color': 'red', 'fontWeight': 'bold'}
        )
    ...
    return html.Div([
        ...
        html.Div([
            ...
            dcc.Dropdown(
                ...
                options=default_value_options,  # Phase 2+: 初期オプションを設定
                value=default_value,  # デフォルト値を設定
                style={
                    'width': '300px',
        ...
        dcc.Loading(
            ...
            children=html.Div([
                html.Div(
                    ...
                    children=default_content  # 初期コンテンツを設定
                ),
...
# ===== Phase 2: Commented out update_comparison_heatmaps (Blueprint/Heatmap/Optimization callback) =====
# @app.callback(
...
def register_shortage_callbacks(app_instance):
    """
    不足率分析コールバックを指定されたアプリインスタンスに登録する

    この関数は2つのコールバックを登録します:
    - update_shortage_heatmap_detail: 詳細選択ドロップダウンを更新
    - update_shortage_ratio_heatmap: 不足率ヒートマップの可視化をレンダリング

    Args:
        app_instance: コールバックを登録するDashアプリインスタンス
    """
    ...
    @safe_callback
    def update_shortage_heatmap_detail(scope, session_id, metadata):
        """不足率ヒートマップの詳細選択を更新"""
        # Session ID が未設定の場合は何も返さない
        if not session_id:
            return None

        # Session と scenario dir を設定
        session = get_session(session_id)
        if not session:
...
def update_optimization_content(scope, detail_values):
    """
    最適化分析コンテンツを更新（Pattern A対応）

    Pattern A実装:
    - ロジックは全てヘルパー関数に委譲
    - グローバル変数不使用、セッション分離維持
    """
...
# )
# ===== COMMENTED OUT: update_overview_insights (Phase 3.1: Legacy callback disabled after Phase 2+) =====
# @safe_callback
...
#     Input('kpi-data-store', 'data'),
# )
# ===== COMMENTED OUT: update_shortage_insights (Phase 3.1: Legacy callback disabled after Phase 2+) =====
# @safe_callback
...
#     Input('kpi-data-store', 'data'),
# )
# ===== COMMENTED OUT: update_hire_plan_insights (Phase 3.1: Legacy callback disabled after Phase 2+) =====
# @safe_callback
...
#     Input('kpi-data-store', 'data'),
# )
# ===== COMMENTED OUT: update_optimization_insights (Phase 3.1: Legacy callback disabled after Phase 2+) =====
# @safe_callback
...
#     Input('kpi-data-store', 'data'),
# )
# ===== COMMENTED OUT: update_leave_insights (Phase 3.1: Legacy callback disabled after Phase 2+) =====
# @safe_callback
...
#     Input('kpi-data-store', 'data'),
# )
# ===== COMMENTED OUT: update_cost_insights (Phase 3.1: Legacy callback disabled after Phase 2+) =====
# @safe_callback
...
# ===== Phase 1: Commented out update_cost_analysis_content (disabled tab callback) =====
# @app.callback(
...
#         raise PreventUpdate
#
#     log.info(f"[Cost] Callback: by_key={by_key}, wages count={len(all_wages)}")
#
#     # 単価辞書作成
#     wages = {
...
#
#     # ヘルパー関数に全ロジックを委譲
#     return _generate_cost_analysis_content(by_key, wages)

# Phase 2: 
# Phase 2: @app.callback(
# Phase 2:     Output('clear-synergy-cache-btn', 'children'),
# Phase 2:     Input('clear-synergy-cache-btn', 'n_clicks'),
# Phase 2:     prevent_initial_call=True
# Phase 2: )
# Phase 2: @safe_callback
# Phase 2: def clear_synergy_cache_callback(n_clicks):
# Phase 2:     """シナジーキャッシュクリアボタンのコールバック"""
# Phase 2:     if n_clicks:
# Phase 2:         clear_synergy_cache()
# Phase 2:         return "キャッシュクリア済み"
...
# ===== COMMENTED OUT: update_individual_analysis_content (Phase 3.1: Legacy callback disabled after Phase 2+) =====
# @safe_callback
...
# ===== COMMENTED OUT: update_team_analysis_graphs (Phase 3.1: Legacy callback disabled after Phase 2+) =====
# @safe_callback
# def update_team_analysis_graphs(selected_value, selected_key):
#     """
#     チーム条件変更時にコンテンツを更新（Pattern A対応）
# 
#     Pattern A実装:
#     - ロジックは全てヘルパー関数に委譲
#     - グローバル変数不使用、セッション分離維持
...
#         raise PreventUpdate
# 
#     log.info(f"[Team] Callback: チーム条件変更 = {selected_key}:{selected_value}")
# 
    # ヘルパー関数に全ロジックを委譲
#     return _generate_team_analysis_content(selected_key, selected_value)
# 
# 
@app.callback(
...
def update_blueprint_analysis_content(n_clicks, analysis_type, session_id, metadata):
    ...
    print("="*80)
    print("[Blueprint DEBUG] CALLBACK INVOKED!")
    print(f"  n_clicks={n_clicks}")
    print(f"  analysis_type={analysis_type}")
    print(f"  session_id={session_id}")
    print(f"  metadata={metadata}")
    print("="*80)
    log.info(f"[Blueprint] ブループリント分析開始: session_id={session_id}, analysis_type={analysis_type}")
    ...
    session = get_session(session_id)
    if not session:
        log.error(f"[Blueprint] セッションが見つかりません: session_id={session_id}")
        empty_fig = go.Figure()
        empty_fig.update_layout(
            title="セッションエラー",
            annotations=[dict(
                text="セッションが見つかりません。ページを再読み込みしてください。",
                xref="paper", yref="paper", x=0.5, y=0.5,
                xanchor='center', yanchor='middle', showarrow=False,
                font=dict(size=14, color="#d32f2f")
            )]
        )
        error_message = html.Div("セッションエラー: データを再読み込みしてください",
    ...
    if not scenario:
        ...
        empty_fig.update_layout(
            title="シナリオエラー",
            annotations=[dict(
                text=f"シナリオ '{scenario_name}' が見つかりません。",
                xref="paper", yref="paper", x=0.5, y=0.5,
                xanchor='center', yanchor='middle', showarrow=False,
                font=dict(size=14, color="#d32f2f")
            )]
        )
        error_message = html.Div("シナリオエラー", style={'color': '#d32f2f', 'padding': '20px'})
        return {}, empty_fig, [], [], [], error_message, error_message
    ...
    log.info(f"[Blueprint] セッション情報設定完了: scenario_path={scenario.root_path}")
    ...
    try:
        ...
        try:
            current_dir = _get_current_scenario_dir()
            if current_dir and current_dir.exists():
                report_file = create_dashboard_analysis_report(current_dir, analysis_type="BLUEPRINT")
                if report_file:
...
# ===== Phase 2: Commented out filter_facts_by_category (Blueprint callback) =====
# @app.callback(
#     Output('facts-data-table', 'data', allow_duplicate=True),
...
# ===== Phase 2: Commented out update_staff_view (Blueprint callback) =====
# @app.callback(
...
# 孤立したcallback: レイアウトに対応するUIコンポーネントが存在しないためコメントアウト
# TODO: 採用シミュレーション機能を実装する場合は、UIコンポーネントを追加してから有効化
# @app.callback(
...
# )
# @safe_callback
def _unused_update_hire_simulation(selected_pattern, added_fte, kpi_data):
...
# ===== Phase 1: Commented out update_logic_analysis_immediate (disabled tab callback) =====
# @app.callback(
...
# ===== Phase 1: Commented out run_deep_analysis_background (disabled tab callback) =====
# @app.callback(
...
# 孤立したcallback: レイアウトに対応するUIコンポーネントが存在しないためコメントアウト
# TODO: 進捗バー機能を実装する場合は、UIコンポーネントを追加してから有効化
# @app.callback(
...
# )
# @safe_callback
def _unused_update_progress_bar(n_intervals, progress_data):
...
# ===== Phase 1: Commented out execute_mind_reader_analysis (disabled tab callback) =====
...
# ============================================================================
# SESSION-AWARE PAGE WRAPPER FUNCTIONS
# ============================================================================

def page_overview(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Overview tab wrapper - bridges session interface to existing create_overview_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
...
def page_heatmap(session: SessionData, metadata: Optional[dict]) -> html.Div:
    ...
    scenario_name = metadata.get("scenario") if metadata else None
    print(f"DEBUG [page_heatmap]: scenario_name={scenario_name}, metadata={metadata}")
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
...
def page_shortage(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Shortage analysis tab wrapper - bridges session interface to existing create_shortage_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_individual(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Individual staff analysis tab wrapper - bridges session interface to existing create_individual_analysis_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_team(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Team analysis tab wrapper - bridges session interface to existing create_team_analysis_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_fatigue(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Fatigue analysis tab wrapper - bridges session interface to existing create_fatigue_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_leave(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Leave analysis tab wrapper - bridges session interface to existing create_leave_analysis_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_fairness(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Fairness analysis tab wrapper - bridges session interface to existing create_fairness_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_optimization(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Optimization tab wrapper - bridges session interface to existing create_optimization_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_forecast(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Forecast tab wrapper - bridges session interface to existing create_forecast_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_hire_plan(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Hire plan tab wrapper - bridges session interface to existing create_hire_plan_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_cost(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Cost analysis tab wrapper - bridges session interface to existing create_cost_analysis_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_gap_analysis(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Gap analysis tab wrapper - bridges session interface to existing create_gap_analysis_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_blueprint(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Blueprint analysis tab wrapper - bridges session interface to existing create_blueprint_analysis_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_mind_reader(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Mind reader tab wrapper - bridges session interface to existing AI analysis tab."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_summary(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """Summary report tab wrapper - bridges session interface to existing create_summary_report_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
def page_reports(session: SessionData, metadata: Optional[dict]) -> html.Div:
    """PPT report tab wrapper - bridges session interface to existing create_ppt_report_tab()."""
    scenario_name = metadata.get("scenario") if metadata else None
    session_id = metadata.get("token") if metadata else None  # Use token as session_id
    _, scenario = session.get_scenario_data(scenario_name)
    old_dir = _get_current_scenario_dir()
...
# ============================================================================
# END SESSION-AWARE PAGE WRAPPER FUNCTIONS
# ============================================================================


# ============================================================================
# PHASE 2: INTERACTIVE CALLBACKS REGISTRATION
# ============================================================================

def register_interactive_callbacks(app_instance):
    """
    Phase 2: インタラクティブ機能用のcallbackを登録

    このまずはPhase 2の全実装が完了するまでのプレースホルダーとして、
    基本的な構造のみを定義します。各タブのcallbackは順次実装します。

    Args:
        app_instance: run_dash_server.pyのDashアプリインスタンス
    """
    from dash.dependencies import Input, Output, State, ALL
    from dash.exceptions import PreventUpdate

    log.info("[Phase 2] Registering interactive callbacks...")

    # デバッグ: callback登録前の状態確認
    log.info(f"[Phase 2 Debug] App callback map keys: {list(app_instance.callback_map.keys()) if hasattr(app_instance, 'callback_map') else 'N/A'}")
    ...
    @app_instance.callback(
        Output('heatmap-area1', 'children'),
        [Input('heatmap-role1', 'value'),
         Input('heatmap-emp1', 'value')],
        [State('session-id', 'data'),
         State('session-metadata', 'data')],
        prevent_initial_call=True
    )
    def update_heatmap_area1(role, emp, session_id, metadata):
        """Heatmap エリア1の動的更新 (Phase 2+ エラーハンドリング付き)"""
        ...
        if role is None or emp is None:
            raise PreventUpdate

        try:
            log.info(f"[Heatmap Area1] Updating: role={role}, emp={emp}, session={session_id}")
            return _generate_static_heatmap(role, emp)
        except Exception as e:
            log.error(f"[Heatmap Area1] Error: {str(e)}", exc_info=True)
            return html.Div([
                html.H4("エラーが発生しました", style={'color': 'red'}),
                html.P(f"ヒートマップの生成中にエラーが発生しました: {str(e)}"),
                html.P("データが正しくアップロードされているか確認してください。", style={'color': 'gray'})
            ], style={'padding': '20px', 'border': '1px solid red', 'borderRadius': '5px'})

    @app_instance.callback(
        Output('heatmap-area2', 'children'),
        [Input('heatmap-role2', 'value'),
         Input('heatmap-emp2', 'value')],
        [State('session-id', 'data'),
         State('session-metadata', 'data')],
        prevent_initial_call=True
    )
    def update_heatmap_area2(role, emp, session_id, metadata):
        """Heatmap エリア2の動的更新 (Phase 2+ エラーハンドリング付き)"""
        # Sessionとscenario dirを設定
        session = get_session(session_id)
        if not session:
            return html.Div("セッションが見つかりません。再度ZIPファイルをアップロードしてください。")
        ...
        try:
            log.info(f"[Heatmap Area2] Updating: role={role}, emp={emp}, session={session_id}")
            return _generate_static_heatmap(role, emp)
        except Exception as e:
            log.error(f"[Heatmap Area2] Error: {str(e)}", exc_info=True)
            return html.Div([
                html.H4("エラーが発生しました", style={'color': 'red'}),
                html.P(f"ヒートマップの生成中にエラーが発生しました: {str(e)}"),
                html.P("データが正しくアップロードされているか確認してください。", style={'color': 'gray'})
    ...
    def update_individual_content(staff, session_id, metadata):
        ...
        try:
            ...
            return html.Div([
                html.H4("エラーが発生しました", style={'color': 'red'}),
                html.P(f"スタッフ「{staff}」の分析中にエラーが発生しました: {str(e)}"),
                html.P("他のスタッフを選択するか、データを確認してください。", style={'color': 'gray'})
            ], style={'padding': '20px', 'border': '1px solid red', 'borderRadius': '5px'})
    ...
    def update_team_value_options(key, session_id, metadata):
        """Team Tab: key選択に応じてvalueのオプションを動的更新 (Phase 2+ エラーハンドリング付き)

        Phase 2+: key変更時にvalueのオプションを更新

        Args:
            key: 選択されたkey ('role', 'code', 'employment' など)
            session_id: セッションID
            metadata: セッションメタデータ

        Returns:
            List[dict]: valueドロップダウンのオプションリスト
        """
        # Sessionとscenario dirを設定
        session = get_session(session_id)
        if not session:
        ...
        try:
            ...
            if long_df.empty:
                log.warning("[Team] long_df is empty - returning empty options")
                return []

            if key is None or key not in long_df.columns:
                log.warning(f"[Team] Invalid key={key} - returning empty options")
                return []
            ...
            try:
                ...
                log.warning(f"[Team] Failed to sort values: {e}")
                sorted_values = unique_values

            options = [{'label': str(val), 'value': val} for val in sorted_values]

            log.info(f"[Team] Value options updated for key={key}: {len(options)} options")
            return options
        except Exception as e:
            log.error(f"[Team] Error updating value options for key={key}: {str(e)}", exc_info=True)
            return []
    ...
    def update_opt_detail_filter(scope, session_id, metadata):
        ...
        Args:
            scope: 選択されたscope ('overall', 'role', 'employment')
            session_id: セッションID
            metadata: セッションメタデータ

        Returns:
            Optional[dcc.Dropdown]: 詳細フィルタードロップダウン、またはNone
        """
        # Sessionとscenario dirを設定
        session = get_session(session_id)
        if not session:
        ...
        try:
            if scope == 'overall':
                log.info("[Optimization] Scope=overall: No detail filter needed")
                return None  # 全体表示の場合は詳細フィルター不要

            long_df = data_get('long_df', pd.DataFrame())
            if long_df.empty:
                log.warning("[Optimization] long_df is empty - no detail filter")
                return None

            if scope == 'role':
                if 'role' not in long_df.columns:
                    log.warning("[Optimization] 'role' column not found")
                    return None
                ...
                return html.Div([
                    html.Label("職種を選択:", style={'marginBottom': '5px'}),
                    dcc.Dropdown(
                        id={'type': 'opt-detail', 'index': 'role'},
                ...
                if 'employment' not in long_df.columns:
                    log.warning("[Optimization] 'employment' column not found")
                    return None
                ...
                return html.Div([
                    html.Label("雇用形態を選択:", style={'marginBottom': '5px'}),
                    dcc.Dropdown(
                        id={'type': 'opt-detail', 'index': 'employment'},
                        options=[{'label': e, 'value': e} for e in sorted_employments],
            ...
            log.error(f"[Optimization] Error updating detail filter for scope={scope}: {str(e)}", exc_info=True)
            return None
    ...
    def update_optimization_content(scope, detail_values, session_id, metadata):
        ...
        try:
            log.info(f"[Optimization] Updating: scope={scope}, detail_values={detail_values}, session={session_id}")
            return _generate_optimization_content(scope, detail_values)
        except Exception as e:
            log.error(f"[Optimization] Error for scope={scope}: {str(e)}", exc_info=True)
            return html.Div([
                html.H4("エラーが発生しました", style={'color': 'red'}),
                html.P(f"最適化分析「{scope}」の生成中にエラーが発生しました: {str(e)}"),
                html.P("他のスコープを選択するか、データを確認してください。", style={'color': 'gray'})
            ], style={'padding': '20px', 'border': '1px solid red', 'borderRadius': '5px'})
    ...
    def update_wage_inputs(by_key, session_id, metadata):
        """Cost Tab: by_key変更時に単価入力欄を動的生成 (Phase 2+ エラーハンドリング付き)

        Phase 2+: 集計軸変更時に対応する単価入力欄を生成

        Args:
            by_key: 選択された集計軸 ('role', 'employment', 'staff')
            session_id: セッションID
            metadata: セッションメタデータ

        Returns:
            html.Div: 単価入力欄のコンテナ、またはNone
        """
        # Sessionとscenario dirを設定
        session = get_session(session_id)
        if not session:
        ...
        try:
            long_df = data_get('long_df', pd.DataFrame())
            if long_df.empty:
                log.warning("[Cost] long_df is empty - no wage inputs")
                return None
            ...
            sorted_categories = sorted(categories, key=str)
            log.info(f"[Cost] Generating {len(sorted_categories)} wage inputs for by_key={by_key}")

            # 単価入力欄を生成
            inputs = [html.Label(f"{label}:", style={'marginTop': '15px', 'marginBottom': '10px', 'fontWeight': 'bold'})]

            for cat in sorted_categories:
                inputs.append(
                    html.Div([
                        html.Label(f"{cat}:", style={'display': 'inline-block', 'width': '200px'}),
                        dcc.Input(
                            id={'type': 'wage-input', 'index': cat},
                            type='number',
                            placeholder='時給を入力',
                            value=1500,  # デフォルト時給
                            min=0,
                        ...
                        html.Span("円/時間", style={'marginLeft': '5px'})
                    ], style={'marginBottom': '8px'})
                )

            return html.Div(inputs, style={'marginBottom': '20px'})
        except Exception as e:
            log.error(f"[Cost] Error generating wage inputs for by_key={by_key}: {str(e)}", exc_info=True)
            return None
    ...
    def update_cost_analysis_content(by_key, all_wages, all_wage_ids, session_id, metadata):
        ...
        try:
            # 単価辞書の構築
            wages = {}
            if all_wages and all_wage_ids:
                wages = {wage_id['index']: (wage_val or 0) for wage_id, wage_val in zip(all_wage_ids, all_wages)}

            log.info(f"[Cost] Updating: by_key={by_key}, wages count={len(wages)}, session={session_id}")
            return _generate_cost_analysis_content(by_key, wages)
        except Exception as e:
            log.error(f"[Cost] Error for by_key={by_key}: {str(e)}", exc_info=True)
            return html.Div([
                html.H4("エラーが発生しました", style={'color': 'red'}),
                html.P(f"コスト分析「{by_key}」の生成中にエラーが発生しました: {str(e)}"),
                html.P("他の集計軸を選択するか、データを確認してください。", style={'color': 'gray'})
            ], style={'padding': '20px', 'border': '1px solid red', 'borderRadius': '5px'})

    log.info("[Phase 2] Interactive callbacks registered successfully")
