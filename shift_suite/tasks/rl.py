...
if RL_AVAILABLE:
    ...
    # Stub implementation when RL packages are not available
    class RosterEnv:
        """Stub implementation when gymnasium is not available."""
        def __init__(self, *args, **kwargs):
            raise ImportError(
                "RosterEnv requires gymnasium and stable-baselines3. "
...
def learn_roster(
    ...
    *,
    ...
    if not RL_AVAILABLE:
        log.warning(
            "[rl] gymnasium and stable-baselines3 are not installed. "
            "RL functionality is disabled."
        )
        return None
